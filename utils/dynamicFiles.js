...
const generateNavbarFile = (content) => {
  ...
  if (content.page === 'single') {
    ...
    links = `enabledPages.map(
      page => <li key={page}><Link to={page} data-nav-section={page} onClick={handleClick} className={\`\${page === activePage ? 'active': ''} px-[1em] py-[.8em] inline-block rounded-full m-[1px] transition-all duration-500 hover:bg-accent-200/20 hover:text-accent-800 hover:font-semibold\`}>{page}</Link></li>
    )`
  }
  ...
  const syncNavWithScroll = (
    ...
    }, [setActivePage, enabledPages, isAutoScrolling, activePage])
    `
  )

  return `import { useState, useEffect, useContext } from 'react'
...
const Navbar = () => {  
  ...
  const enabledPages = ${JSON.stringify(enabledPages)}
  ${content.page === 'single' ? 'const [isAutoScrolling, setIsAutoScrolling] = useState(false)' : ''}
  ...
  const handleClick = (e) => {
    ${content.page === 'single' ? scrollToSection : 'window.scrollTo({ top: 0 })'}
...
.navbar .active {
  @apply font-semibold text-accent-800;
}
